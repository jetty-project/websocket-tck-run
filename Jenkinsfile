...
pipeline {
  ...
  parameters {
    ...
    string( defaultValue: '2.2.0', description: 'TCK Version',
            name: 'TCK_VERSION' )
...
//    }
    ...
    stage("Install TCK") {
      steps {
        sh 'wget -O jakarta-websocket-tck.zip https://download.eclipse.org/jakartaee/websocket/2.2/jakarta-websocket-tck-2.2.0.zip'
      ...
      steps {
        timeout(time: 90, unit: 'MINUTES') {
          withEnv(["JAVA_HOME=${tool "$JDKBUILD"}",
            ...
            configFileProvider([configFile(fileId: 'oss-settings.xml', variable: 'GLOBAL_MVN_SETTINGS')]) {
              sh "mvn -nsu -ntp -s $GLOBAL_MVN_SETTINGS -Dmaven.test.failure.ignore=true -V -B -U clean verify -e -Dtck.verion=$TCK_VERSION -Djetty.version=$JETTY_VERSION"
            }
          }
        }
      }
